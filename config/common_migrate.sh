...
for b in sakai_11-x_prod sakai_11-x_dev; do
...
# migrate tool
git svn init $INIT_OPTS --branches=REPLACEHERE $SVNROOT $WORKDIR/common.fromsvn

cd "$WORKDIR/common.fromsvn"
sed -i "s|REPLACEHERE/\*|branches/\{$svnbranches\}/server/properties|" .git/config
